...
function displayPost(snapVisitor, snapHost, el) {
    ...
    database.ref().child('items/' + key).once('value', function (snapItem) {
        ...
        $("#postHostName").text(snapHost.val().name);
        $("#open-post #postCaption").text(itemObj["caption"]);
        $("#postInterestsCount").text(itemObj["interests_count"]);
        $("#postCommentsCount").text(itemObj["comments_count"]);
        $("#postSharesCount").text(itemObj["shares_count"]);
        $("#postHostUrl").attr("src", snapHost.val().photo_url);
        $("#postVisitorUrl").attr("src", snapVisitor.val().photo_url);
        $("#postTime").text(timeDisplay(itemObj["time"]));
